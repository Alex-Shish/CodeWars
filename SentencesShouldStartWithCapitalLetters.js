...
You will be given a paragraph that does not use capital letters. Your job is to capitalise the first letter of
...
function fix(paragraph){
    if(!paragraph) return ''
    return paragraph.split('. ').map(el => el[0].toUpperCase() + el.slice(1)).join('. ')
}
