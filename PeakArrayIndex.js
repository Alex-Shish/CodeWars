// Peak array index
// DESCRIPTION:
// Given an array of ints, return the index such that the sum of the elements to the right of that index equals
// the sum of the elements to the left of that index. If there is no such index, return -1.
...
function peak(arr) {
    for (let i = 0; i < arr.length; i++) {
        const sumBegin = arr.slice(0, i).reduce((acc, cur) => acc + cur, 0);
        const sumEnd = arr.slice(i + 1).reduce((acc, cur) => acc + cur, 0);
        if (sumBegin === sumEnd) return i;
    }
    return -1;
}
