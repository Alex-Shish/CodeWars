...
function solve(s){
    let letterUp = s.split('').filter(el => el.match(/[A-Z]/)).length
    let letterDown = s.split('').filter(el => el.match(/[a-z]/)).length
    return letterDown >= letterUp ? s.toLowerCase() : s.toUpperCase();
}
